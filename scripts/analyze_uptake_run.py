#!/usr/bin/env python3
"""Create a compact convergence plot for an aemwater uptake run."""
...
def _dry_atom_counts(run_dir: Path) -> tuple[int, int]:
    ...
    # Counterions are dry-system atoms but not polymer atoms. Older runs may
    # lack composition.json; in that case the dry atom count is the best
    # available non-water count.
...
def plot_run(run_dir: Path, output: Path, water_mu: float | None = None) -> Path:
    """Plot chemical-potential and water-uptake convergence for *run_dir*."""
    ...
    ax_mu.errorbar(
        frame.loc[valid, "index"], frame.loc[valid, "mu_ex"], yerr=yerr,
        fmt="o-", color=blue, lw=1.6, ms=4, capsize=3,
        label="membrane water",
    )
    ax_mu.axhline(
        reference_mu, color=red, ls="--", lw=1.5,
    ...
    ax_uptake.plot(
        frame["index"], frame["water_uptake_pct"], "o-",
        color=blue, lw=1.6, ms=4,
    )
    ax_uptake.set(
        xlabel="iteration",
        ylabel="water uptake (wt %)",
        title="Water uptake",
    )
    ...
    ax_uptake.text(
        0.03, 0.97, count_text, transform=ax_uptake.transAxes,
    ...
    for iteration, uptake, count in zip(
        ...
        ax_uptake.annotate(
            f"{count}", (iteration, uptake), xytext=(0, 7),
            textcoords="offset points", ha="center", fontsize=7, color="0.3",
        )
    ax_uptake.text(
        0.98, 0.03, "labels = water molecules", transform=ax_uptake.transAxes,
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--water-mu", type=float,
        help="bulk-water excess chemical potential in kcal/mol (overrides result.json)",
    )
    return parser
